...
function mla_groups_register_widgets() {
	add_action('widgets_init', create_function('', 'return register_widget("MLA_BP_Groups_Widget");') );
}
add_action( 'bp_register_widgets', 'mla_groups_register_widgets' );

/*** GROUPS WIDGET *****************/

class MLA_BP_Groups_Widget extends WP_Widget {
	function __construct() {
		$widget_ops = array(
			'description' => __( 'A dynamic list of recently active groups, cleaner than the normal groups widget, but not filterable.', 'buddypress' ),
			'classname' => 'widget_mla_bp_groups_widget buddypress widget',
		);
		parent::__construct( false, _x( '(BuddyPress) MLA Groups', 'widget name', 'buddypress' ), $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $instance['group_default'] ) )
			$instance['group_default'] = 'active';

		if ( empty( $instance['title'] ) )
			$instance['title'] = __( 'Recently Active Groups', 'buddypress' );
...
function mla_groups_ajax_widget_groups_list() {
...
add_action( 'wp_ajax_widget_groups_list',        'mla_groups_ajax_widget_groups_list' );
add_action( 'wp_ajax_nopriv_widget_groups_list', 'mla_groups_ajax_widget_groups_list' );
